...
function Paddle:init(paddleIndex, x)
    -- start us off with no velocity
    self.dx = 0

    -- index of the paddle to select the correct paddle from the table
    self.paddleIndex = paddleIndex

    -- starting dimensions
    self.width = gFrames['paddles'][self.paddleIndex]['size']
    self.height = 16
    ...
    -- check if the paddle can destroy the key brick
    self.keypower = false
end
...
function Paddle:render()
    love.graphics.draw(gTextures['main'], gFrames['paddles'][self.paddleIndex]['quad'],
        self.x, self.y)
end