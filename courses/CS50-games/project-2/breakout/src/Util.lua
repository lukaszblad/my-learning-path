...
function GenerateQuadsBricks(atlas)
    local temp_table = table.slice(GenerateQuads(atlas, 32, 16), 1, 21)
    table.insert(temp_table, GenerateQuads(atlas, 32, 16)[24])
    return temp_table
end
...
function GenerateQuadsPaddles(atlas)
    ...
    for i = 0, 3 do
        -- smallest
        quads[counter] = {
            ['quad'] = love.graphics.newQuad(x, y, 32, 16,
            atlas:getDimensions()),
            ['size'] = 32 
        }
        counter = counter + 1
        -- medium
        quads[counter] = {
            ['quad'] = love.graphics.newQuad(x + 32, y, 64, 16,
            atlas:getDimensions()),
            ['size'] = 64
        }
        counter = counter + 1
        -- large
        quads[counter] = {
            ['quad'] = love.graphics.newQuad(x + 96, y, 96, 16,
            atlas:getDimensions()),
            ['size'] = 96
        }
        counter = counter + 1
        -- huge
        quads[counter] = {
            ['quad'] = love.graphics.newQuad(x, y + 16, 128, 16,
            atlas:getDimensions()),
            ['size'] = 128
        }
        counter = counter + 1
