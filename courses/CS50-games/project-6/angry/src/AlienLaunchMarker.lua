...
function AlienLaunchMarker:init(world)
    ...
    -- player's aliens table
    self.aliens = {}

    -- whether alien collided
    self.collided = false

    -- whether alien splitted
    self.splitted = false
end
...
function AlienLaunchMarker:update(dt)
    ...
    elseif not self.collided and not self.splitted then
        -- spawn two more aliens
        if love.keyboard.keysPressed['space'] then
            -- distance between splitted aliens
            local distance = 30
            -- angle between splitted aliens
            local angle = 50
            for i = -1, 1, 2 do
                local alien = Alien(self.world, 'round', self.aliens[1].body:getX(), self.aliens[1].body:getY() + i * distance, 'Player')
                local x, y = self.aliens[1].body:getLinearVelocity()
                alien.body:setLinearVelocity(x, y + i * angle)
                alien.fixture:setRestitution(0.4)
                alien.body:setAngularDamping(1)
                table.insert(self.aliens, alien)
            end
...
function AlienLaunchMarker:render()
    ...
    -- debug print
    -- love.graphics.printf('COLLIDED: ' .. tostring(self.collided), 0, 0, VIRTUAL_WIDTH, 'center')
    -- love.graphics.printf('SPLITTED: ' .. tostring(self.splitted), 0, 20, VIRTUAL_WIDTH, 'center')
end