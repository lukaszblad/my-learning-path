...
function Level:init()
    ...
    function beginContact(a, b, coll)
        ...
        -- change collide flag if there are player's aliens
...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        for k, alien in pairs(self.launchMarker.aliens) do
            -- check for the minimum position and velocity
            ...
            if (xPos < 0) or (xPos > VIRTUAL_WIDTH) or (math.abs(xVel) + math.abs(yVel) < 1.5) then
                ...
                if #self.launchMarker.aliens < 1 then
                    self.launchMarker = AlienLaunchMarker(self.world)
                end
...
function Level:render()
    ...
    -- debug print
    -- love.graphics.printf('ALIENS: ' .. tostring(#self.launchMarker.aliens), 0, 30, VIRTUAL_WIDTH, 'center')
end